...
namespace redfish
{
    public partial class MainWindow : Window
    {
        ...
        {
            if (_serviceWindow != null && _serviceWindow.IsVisible)
            {
                _serviceWindow.Close();
                _serviceWindow = null;
                return;
            }
            else
            {
                // close other windows
                if (_ahkManagerWindow != null && _ahkManagerWindow.IsVisible)
                {
                    _ahkManagerWindow.Close();
                    _ahkManagerWindow = null;
                }
                if (_swapWindow != null && _swapWindow.IsVisible)
                {
                    _swapWindow.Close();
                    _swapWindow = null;
                }

                _serviceWindow = new ServiceWindow()
                {
            ...
            //ServiceButton.FillColor = _serviceWindow != null ? Pin.GlowColor : (SolidColorBrush)Application.Current.FindResource("InactiveColor");
        }
        private void SwapWindowButtonClick(object sender, RoutedEventArgs e)
        {
            if (_swapWindow != null && _swapWindow.IsVisible)
            {
                _swapWindow.Close();
            ...
            //SwapButton.FillColor = _swapWindow != null ? Pin.GlowColor : (SolidColorBrush)Application.Current.FindResource("InactiveColor");
        }
        private void AhkClick(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                ...
                // open new and make icon active
                _ahkManagerWindow = new AhkManagerWindow()
                {
                    Left = this.Left - this.Width - 8,
                    Top = this.Top
                }; 
                _ahkManagerWindow.Show();
            }
            //AhkButton.FillColor = _ahkManagerWindow != null ? Pin.GlowColor : (SolidColorBrush)Application.Current.FindResource("InactiveColor");
        }
        ...
        {
            // FIX: Use IsChecked property correctly
            InstanceModule.RateLimitingEnabled = InstSlowCB.Checked == true;
            Config.GetNamed("З0k").Settings["RateLimitingEnabled"] = InstanceModule.RateLimitingEnabled;
            Config.Save();

            Debug.WriteLine($"Rate limiting enabled: {InstanceModule.RateLimitingEnabled}");
        }
