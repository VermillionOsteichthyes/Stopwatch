    ...
    public partial class OverlayWindow : Window
    {
        ...
        public bool TryFollowWindow()
        {
            if (GetWindowRect(cachedProcess.MainWindowHandle, out var rect))
            {
                ...
                //Debug.WriteLine($"{win_w}/{win_h} -> {Width}:{Height}, xoff-{xoffset}, yoff-{yoffset}");
        ...
        private void CheckInstanceTimer()
        {
            ...
            if (Config.Instance.Settings.Overlay_ShowTime)
            {
                ...
                Timer.Content = DateTime.Now.ToString("mm':'ss");
                Timer.ElementFadeIn();
            }
            else 
            {
                ...
                Timer.Content = $"{duration.Minutes:D2}:{duration.Seconds:D2}";
                Timer.ElementFadeIn();
