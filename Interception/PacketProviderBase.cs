...
namespace redfish.Interception
{
    public abstract class PacketProviderBase : IDisposable
    {
        ...
        public async Task SendPacket(Packet packet, bool force = false)
        {
            ...
            {
                Debug.WriteLine($"{Name} provider: Unable to inject the packet");
                return;
            }
        ...
        private async Task PacketPoll(CancellationToken ct)
        {
            ...
            Debug.WriteLine($"{Name} provider: Started");
            while (!ct.IsCancellationRequested)
            {
                ...
                catch (Exception e)
                {
                    ...
                    Debug.WriteLine($"{Name} provider: {e}");
                }
            }
            ...
            packet.Dispose();
            addr.Dispose();
            Debug.WriteLine($"{Name} provider: Stopped");
        }
        private async Task SavePoll(CancellationToken ct)
        {
            while (CurrentQueue is null || Connections is null)
            {
                ...
                Debug.WriteLine($"{Name} provider: Awaiting variables initialization");
            }

            Debug.WriteLine($"{Name} provider: Started saving packets");
            while (!ct.IsCancellationRequested)
            {
                try
                {
                ...
                {
                    Debug.WriteLine($"{Name} provider: Packet save {e}");
                }
            }

            Debug.WriteLine($"{Name} provider: Stopped saving packets");
        }
        private async Task DelayPoll(CancellationToken ct)
        {
            Delay = new List<Packet>();

            Debug.WriteLine($"{Name} provider: Delay poll started");
            while (!ct.IsCancellationRequested)
            {
                try
                {
                ...
                {
                    Debug.WriteLine($"{Name} delay: {e}");
                }
            }

            ClearDelayQueue();
            Debug.WriteLine($"{Name} delay: Stopped");
        }
        ...
        public void DelayPacket(Packet packet, TimeSpan delay = default, bool addFromLatest = false, bool sameDirection = false)
        {
            if (Delay.Count > 2048)
            {
                Debug.WriteLine($"{Name}: Delay buffer is at capacity");
            ...
            //Debug.WriteLine($"{Name}: Delay {(clone.CreatedAt - packet.CreatedAt).TotalSeconds:G}s last:{addFromLatest} dir:{sameDirection}");
        ...
        public async Task ClearDelayQueue(string addr = null, bool sendSaved = false, int delay = 25)
        {
            ...
            if (sendSaved)
            {
                ...
                Debug.WriteLine($"{Name}: Sent {copy.Count} packets from buffer");
            }
            else
            {
