...
namespace redfish.Interception.Modules
{
    public abstract class PacketModuleBase : IDisposable
    {
        ...
        // Display name is what is shown to users, can be localized
        public virtual string DisplayName => Name; // defaults to internal name
        ...
        private void KeybindPressedHandler(LinkedList<Keycode> keycodes)
        {
            ...
            Task.Run(() =>
            {
                Debug.WriteLine($"{Name}: Hotkey fired");
                ...
                if (Togglable)
                {
                    if (!IsActivated) Debug.WriteLine($"{Name}: Worked for {DateTime.Now - savedTime}");
        ...
        public void ForceDisable()
        {
            ...
            if (Togglable)
            {
                ...
                Debug.WriteLine($"{Name}: Worked for {DateTime.Now - StartTime}");
