...
namespace redfish.Interception.PacketProviders
{
    public class PlayersProvider : PacketProviderBase
    {
        ...
        public override bool AllowPacket(Packet p)
        {
            ...
            //    {
            //        resync.Ids.Add(id);
            //        Debug.WriteLine($"{Name}: {p.RemoteAddress} Synced with {id}");
            //    }
            //}

            //var text = Encoding.ASCII.GetString(p.Payload);
            //bool id = true;
            //var matches = Regex.Matches(text, @"((?:xbox|psn|xbox|egs)-.{16}|redfishid:\d{17})");
            //if (!matches.Any())
            //{
            //    id = false;
            ...
            //// low redfishid len after closed from local is acknowledgement
            ...
            //        {
            //            Debug.WriteLine($"{Name}: DL:{p.Inbound} {p.RemoteAddress}: Close acknowledge");
            //            return true;
            //        }
            //        Debug.WriteLine($"{Name}: DL:{p.Inbound} {p.RemoteAddress}: Just some id");
            //    }
            ...
            //        Debug.WriteLine($"{Name}: {p.RemoteAddress} {(p.Inbound ? "Remote closed the connection" : "We closed the connection [START]")}");
            //    }
            //    else // ping starts resync
            //    if (first.Contains("ping", StringComparison.OrdinalIgnoreCase))
            //    {
            //        resync.LastStart = p.CreatedAt;
            //        Debug.WriteLine($"{Name}: {p.RemoteAddress} DL:{p.Inbound} Start ping");
            //    }
            //}
            ...
            //{
            //    resync.LastStart = p.CreatedAt;
            //    Debug.WriteLine($"{Name}: {p.RemoteAddress} DL:{p.Inbound} Start from old connection");
            //    return true;
            //}
            ...
            //        Debug.WriteLine($"{Name}: {p.RemoteAddress} DL Finished");
            //    }
            ...
            //        Debug.WriteLine($"{Name}: {p.RemoteAddress} UL Finished");
            //    }
            ...
            //    Debug.WriteLine($"{Name}: {p.RemoteAddress} Finished with fallback [10s]");
            //}
