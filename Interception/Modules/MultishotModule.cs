    ...
    public class MultishotModule : PacketModuleBase
    {
        ...
        private void ShootButtonTracker(LinkedList<Keycode> keycode)
        {
            if (keycode.Any(x => x == Keycode.VK_LMB))
            {
                    ...
                    OverlayWindow.CheckGameFocus())
                {
                    ...
                    Debug.WriteLine($"{Name}: Recent start found");
                }
            }
        }
        ...
        public override void Toggle()
        {
            ...
            if (!WaitShot) // no validation
            {
                if (DateTime.Now - lastLMB < shotTimeout)
                {
                    ...
                    Debug.WriteLine($"{Name}: Recent lmb found");
                }
                else
                {
                    lastShot = DateTime.Now;
            ...
            if (shots.Any())
            {
                ...
                Debug.WriteLine($"{Name}: Found recent shot, starting immediately");
            }
        }
        ...
        public override bool AllowPacket(Packet p)
        {
            ...
            if (block)
            {
                ...
                {
                    Debug.WriteLine($"{Name}: Encountered reconnect packet [{DateTime.Now - StartTime}]");
                    Disable();
                    return false;
                }
                ...
                {
                    Debug.WriteLine($"{Name}: No shots are being made [{DateTime.Now - StartTime}]");
                    Disable();
                    return false;
                }
                ...
                {
                    Debug.WriteLine($"{Name}: Timelimit hit [{DateTime.Now - StartTime}]");
                    Disable();
                    return false;
                }
            }
        ...
        bool Handle3074(Packet p)
        {
            ...
            {
                Disable();
                Debug.WriteLine($"{Name}: Pve reconnect");
                return true;
            }
            ...
            {
                Disable();
                Debug.WriteLine($"{Name}: Drop amount exceeded");
            }
        ...
        bool Handle27K(Packet p)
        {
            ...
            if (shot)
            {
                ...
                Debug.WriteLine($"{Name}: Shot x{addrMap[r]}");
            }
